...
import com.mygame.engine.items.Terrain;
...
public class DummyGame implements IGameLogic {
	...
	public void init(Window window) throws Exception {
		...
		String fileName = Thread.currentThread().getContextClassLoader().getResource("models/terrain/terrain.obj")
				.getFile();
		...
		fileName = Thread.currentThread().getContextClassLoader().getResource("models/bob/boblamp.md5mesh").getFile();
		...
		fileName = Thread.currentThread().getContextClassLoader().getResource("models/house/house.obj").getFile();
		file = new File(fileName);
		...
		float maxY = 0.1f;
		int textInc = 40;
		terrain = new Terrain(terrainSize, terrainScale, minY, maxY, "/textures/heightmap.png", "/textures/terrain.png",
				textInc);

		GameItem[] gameItems = { animItem, house };
		gameItems = ArrayUtils.addAll(gameItems, terrain.getGameItems());
		...
		fileName = Thread.currentThread().getContextClassLoader().getResource("models/skybox/skybox.obj").getFile();
		file = new File(fileName);
	...
	public void update(float interval, MouseInput mouseInput, Window window) {
		...
		if (animItem.getPosition().y + animItem.getVerticalVelocity() < 0.0f) {
			// animItem.setPosition(animItem.getPosition().x, 0.0f,
			// animItem.getPosition().z);
		...
		float height = terrain.getHeight(animItem.getPosition());
		if (animItem.getPosition().y + animItem.getVerticalVelocity() < height) {
			animItem.setPosition(animItem.getPosition().x, height, animItem.getPosition().z);
			animItem.setVerticalVelocity(0.0f);
		}



		animItem.movePosition(
				posInc.z * CAMERA_POS_STEP * (float) Math.cos(Math.toRadians(lookAt.y))
						+ posInc.x * CAMERA_POS_STEP * (float) Math.cos(Math.toRadians(-lookAt.y + 90.0f)),
				posInc.y * CAMERA_POS_STEP, posInc.z * CAMERA_POS_STEP * (float) Math.sin(Math.toRadians(lookAt.y))
						+ posInc.x * CAMERA_POS_STEP * (float) Math.sin(Math.toRadians(lookAt.y - 90.0f)));

		camera.setPosition(
				animItem.getPosition().x + camDistance * (float) Math.cos(Math.toRadians(lookAt.y))
						* (float) Math.cos(Math.toRadians(lookAt.x)),
				1.5f + animItem.getPosition().y + camDistance * (float) Math.sin(Math.toRadians(lookAt.x)),
				animItem.getPosition().z + camDistance * (float) Math.sin(Math.toRadians(lookAt.y))
						* (float) Math.cos(Math.toRadians(lookAt.x)));
